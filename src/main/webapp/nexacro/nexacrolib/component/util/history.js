if(nexacro._Browser!="Runtime") {
	...
	if(window.parent.NEXACRO_PARENT_FRAME) {
		pWindow = window.parent.NEXACRO_PARENT_FRAME;
	} else {
		pWindow = window;
	}
	...
	(function (context, namespace) {
		...
		if ( history.pushState && history.state !== undefined )
		{
			_history = {
				...
				init: function(historyChangeListener, historyChangeListenerScope)
				{
					...
					context.onpopstate = me.checkLocationHash;
				},
				...
				setLocationHash: function(hash, data, url)
				{
					var me = _history;
					...
					context.history.pushState(state, hash, url);
				},
		...
		if(window != context) {	window[namespace] = context[namespace];	}
